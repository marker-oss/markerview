...
type SiteReviewInput struct {
	...
	Media            []ReviewMedia
	// CustomData is the JSON-encoded map of custom-field answers collected by
	// the submission form (see server.NormalizedCustomFields for validation).
	CustomData string
}
...
func (s *Store) CreateSiteReview(ctx context.Context, input SiteReviewInput) (Review, error) {
	...
	err = s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Where("tenant_id = ? AND email_normalized = ?", TenantIDFromCtx(ctx), email).First(&identity).Error; err != nil {
			return err
		}
