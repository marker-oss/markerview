...
func (c *Client) PublishQuestionAnswer(ctx context.Context, externalQuestionID, _ /*sku*/, text string) error {
	...
	resp, err := c.executor.Do(ctx, c.httpClient, apihttp.WBPolicy(), apihttp.Write, func(attemptCtx context.Context) (*http.Request, error) {
		req, err := http.NewRequestWithContext(attemptCtx, http.MethodPatch, c.baseURL+"/api/v1/questions", bytes.NewReader(body))
		if err != nil {
			return nil, err
		}
		req.Header.Set("Authorization", c.token)
...
func (c *Client) PublishReply(ctx context.Context, externalReviewID, text string) error {
	...
	resp, err := c.executor.Do(ctx, c.httpClient, apihttp.WBPolicy(), apihttp.Write, func(attemptCtx context.Context) (*http.Request, error) {
		req, err := http.NewRequestWithContext(attemptCtx, http.MethodPost, c.baseURL+"/api/v1/feedbacks/answer", bytes.NewReader(body))
		if err != nil {
			return nil, err
		}
		req.Header.Set("Authorization", c.token)
