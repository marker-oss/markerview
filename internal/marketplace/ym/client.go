...
func (c *Client) fetchPage(ctx context.Context, cursor string) ([]goodsFeedback, string, error) {
	...
	resp, err := c.executor.Do(ctx, c.httpClient, apihttp.YMPolicy(), apihttp.Read, func(attemptCtx context.Context) (*http.Request, error) {
		req, err := http.NewRequestWithContext(attemptCtx, http.MethodPost, endpoint.String(), strings.NewReader("{}"))
		if err != nil {
			return nil, err
		}
...
func (c *Client) FetchQuestions(ctx context.Context, since time.Time, cursor string) ([]marketplace.Question, string, error) {
	...
	resp, err := c.executor.Do(ctx, c.httpClient, apihttp.YMPolicy(), apihttp.Read, func(attemptCtx context.Context) (*http.Request, error) {
		req, err := http.NewRequestWithContext(attemptCtx, http.MethodPost, endpoint.String(), strings.NewReader(string(rawBody)))
		if err != nil {
			return nil, err
		}
		req.Header.Set("Api-Key", c.apiKey)
...
func (c *Client) fetchQuestionAnswer(ctx context.Context, questionID string) (*marketplace.Answer, error) {
	...
	resp, err := c.executor.Do(ctx, c.httpClient, apihttp.YMPolicy(), apihttp.Read, func(attemptCtx context.Context) (*http.Request, error) {
		req, err := http.NewRequestWithContext(attemptCtx, http.MethodPost, endpoint, strings.NewReader(string(body)))
		if err != nil {
			return nil, err
		}
		req.Header.Set("Api-Key", c.apiKey)
...
func (c *Client) PublishQuestionAnswer(ctx context.Context, externalQuestionID, _ /*sku*/, text string) error {
	...
	resp, err := c.executor.Do(ctx, c.httpClient, apihttp.YMPolicy(), apihttp.Write, func(attemptCtx context.Context) (*http.Request, error) {
		req, err := http.NewRequestWithContext(attemptCtx, http.MethodPost, endpoint, strings.NewReader(string(body)))
		if err != nil {
			return nil, err
		}
