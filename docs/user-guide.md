# Руководство пользователя MarkerView

[О продукте](../README.md) · [Развёртывание](../deploy/README.md) · [Техническая документация](technical/README.md)

Подключение маркетплейсов, сопоставление товаров, работа в админке и установка виджета. Пользовательский self-hosted-путь — Docker Compose; сборка без Docker описана только в документации для разработчиков.

## Docker: подготовка к запуску

Рекомендуемый self-hosted-путь — Docker Compose. Он собирает образ из текущего
checkout, поэтому готовый registry image для этой инструкции не требуется.
...
1. **Docker-хост.** Нужны Docker с Compose plugin, место для постоянных томов и
   доступ к `127.0.0.1:8080` для reverse proxy. Для production также откройте
   входящие `80/443`; сам порт приложения не выставляйте напрямую в Internet.
2. **Домен и DNS.** Создайте A-запись, например
   `reviews.ваш-магазин.ru` → IP сервера. HTTPS-сертификат может автоматически
   получить Caddy; пример конфигурации есть в [`deploy/`](../deploy/).
3. **Origin магазина.** Полный адрес основного сайта, например
   `https://ваш-магазин.ru`, нужен для CORS и ссылок на товары.
4. **Администратор.** При первом открытии `/admin` задайте логин и пароль не
   короче 8 символов.
5. **Ключи подключаемых маркетплейсов.** Их можно задать в `.env` до запуска
   или сохранить через админку после него. Не публикуйте `.env` и вывод
   `docker compose config`: в них могут быть секреты.

Ключи маркетплейсов:

- **Wildberries** — персональный API-токен. ЛК продавца → **Профиль →
  Настройки → Доступ к API** → создать токен с категорией **«Вопросы и
  отзывы»**. Скопируйте сразу — повторно токен не показывается. Базовые,
  тестовые и сервисные токены сервис отклоняет — нужен именно персональный.
  Смена токена применяется сразу, без перезапуска контейнера.
...
2. **Заполните `.env`** (`cp .env.example .env`):
   ...
   - `REVIEWS_PUBLIC_DOMAIN=reviews.ваш-магазин.ru` — публичный домен сервиса;
...
3. **Запустите:** `docker compose up -d --build`, проверьте `GET /healthz`.
   ...
   сертификат Let's Encrypt он выпустит и продлит сам (см. [deploy/](../deploy/)).
...
- **Wildberries** — персональный API-токен с категорией «Отзывы и вопросы»;
  базовые, тестовые и сервисные токены отклоняются. Смена токена применяется
  без перезапуска контейнера; на 429 чтения повторяются один раз, публикация
  ответов не повторяется автоматически;
...
Compose — единственный поддерживаемый пользовательский self-hosted-путь.
Инструкции для локальной разработки без Docker находятся в
[`docs/technical/development.md`](technical/development.md).
...
| `REVIEWS_AUTOPUBLISH_INTERVAL` | `5m` | как часто автопубликация проверяет изменения и перегенерирует статический экспорт. `0`/`off` — отключить |
| `REVIEWS_CATALOG_REFRESH_INTERVAL` | `24h` | период фонового обхода sitemap магазина. `0`/`off` — отключить |
...
> рестарта). При Docker-запуске с reverse proxy отдельная настройка CORS на
> прокси не нужна.
...
1. **Запустить Compose.** При старте контейнер сам применит миграции и включит
   фоновую синхронизацию:
   ```sh
   docker compose up -d --build
   ```
2. **Создать администратора.** Откройте `http://127.0.0.1:8080/admin` — при
   ...
   Альтернатива / сброс пароля через CLI контейнера:
   ```sh
   docker compose run --rm reviews admin reset-password --login admin --password НОВЫЙ_ПАРОЛЬ
   ```
   (сбрасывает пароль и завершает все активные сессии пользователя)

3. **Войти** на `/admin` под созданными логином и паролем.
...
Текущий Compose собирает локальный образ из checkout и не скачивает готовый
образ из registry. Поэтому `docker compose pull` и image-based auto-update для
этого сценария неприменимы.

Перед обновлением сохраните SQLite DB, uploads, каталог и опубликованный экспорт;
процедура backup/restore описана в
[`docs/technical/operations-and-security.md`](technical/operations-and-security.md).

Контейнер distroless не содержит shell. Не копируйте работающую SQLite-базу
обычным `cp`: используйте согласованный snapshot или процедуру из документации
по резервному копированию выше.

**Ручная синхронизация** — из админки (кнопка) или через Compose:
```sh
docker compose run --rm reviews sync --once --marketplace ym
```
...
- [ ] Разместили политику конфиденциальности — шаблон в
      [docs/legal/privacy-policy-template.ru.md](legal/privacy-policy-template.ru.md),
...
Сервис предоставляет средства обезличивания имён, размывания лиц в медиа и
обработки запросов на удаление данных. Это не гарантия юридического соответствия:
права на публикацию, настройки и применимые требования проверяет оператор.
Медиа, отправленные через форму сайта, хранятся локально и требуют резервного
копирования и соблюдения сроков хранения.
