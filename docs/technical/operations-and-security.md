...
Поддерживаемый self-hosted-путь — Docker Compose со сборкой образа из checkout. Source/binary packaging ниже описан только как технический факт и не является вторым пользовательским способом установки.
