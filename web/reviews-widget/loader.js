...
(function () {
  ...
  // Appends the tenant public key to public API URLs when configured. In
  // single-tenant mode the key is absent and the server serves the default
  // tenant.
  function withPublicKey(url) {
    if (CFG.publicKey && url && url.indexOf("public_key=") < 0) {
      return url + (url.indexOf("?") >= 0 ? "&" : "?") + "public_key=" + encodeURIComponent(CFG.publicKey);
    }
    return url;
  }
  ...
  function showcaseUrl() {
    ...
    return withPublicKey(base.replace(/\/$/, "") + "/api/showcase");
  }
  ...
  function reviewsUrl(contextName) {
    ...
    return withPublicKey(url.toString());
  }

  function submissionConfigUrl() {
    return withPublicKey(serviceBase().replace(/\/$/, "") + "/api/review-submission-config");
  }

  function submissionUrl() {
    return withPublicKey(serviceBase().replace(/\/$/, "") + "/api/review-submissions");
  }
  ...
  function loadWidgetConfig(contextName) {
    ...
    var base = serviceBase();
    var url = withPublicKey(base.replace(/\/$/, "") + "/api/widget-config?context=" + encodeURIComponent(contextName));
